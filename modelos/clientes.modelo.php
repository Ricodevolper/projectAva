...
class ModeloClientes
{
    ...
    static public function mdlBuscarCliente($nombre, $perfil)
    {
        try {
            ...
            $nombre = trim(preg_replace('/\s+/', ' ', htmlspecialchars($nombre)));
            $nombre = strtolower($nombre); // Convertir a minúsculas para búsqueda insensible a mayúsculas

            // Definir la consulta en función del perfil
            if ($perfil == 'DirectorAvawm') {
                $query = "SELECT tbl_clientes.*, 
                            CONCAT(tbl_clientes.nombre_clte, ' ', tbl_clientes.apaterno_clte, ' ', tbl_clientes.amaterno_clte) AS cliente,
                            cat_localidad.nom_localidad,
                            cat_municipio.desc_municipio,
                            cat_estado.nom_estado,
                            CONCAT_WS(' ', nombre_promotor, apaterno_promotor, amaterno_promotor) AS nombre_asesor
                        FROM tbl_clientes
                        LEFT JOIN cat_localidad ON tbl_clientes.fk_localidad = cat_localidad.clave_localidad 
                            AND tbl_clientes.fk_pais = cat_localidad.fk_pais 
                            AND tbl_clientes.fk_estado = cat_localidad.fk_estado 
                            AND tbl_clientes.fk_municipio = cat_localidad.fk_municipio
                        LEFT JOIN cat_municipio ON cat_localidad.fk_pais = cat_municipio.fk_pais 
                            AND cat_localidad.fk_estado = cat_municipio.fk_estado 
                            AND cat_localidad.fk_municipio = cat_municipio.clave_municipio
                        LEFT JOIN cat_estado ON cat_municipio.fk_estado = cat_estado.id_estado 
                            AND cat_municipio.fk_pais = cat_estado.fk_pais
                        ...
                        WHERE LOWER(REPLACE(CONCAT(tbl_clientes.nombre_clte, ' ', tbl_clientes.apaterno_clte, ' ', tbl_clientes.amaterno_clte), '  ', ' ')) 
                            LIKE :nombre
                            AND rel_gerente_asesor.fk_id_gerente = 10
    ...
    LEFT JOIN rel_empleado_cliente ON rel_empleado_cliente.fk_cliente = tbl_clientes.id_cliente
    LEFT JOIN tbl_empleados ON tbl_empleados.id_promotor = rel_empleado_cliente.fk_id_empleado
    WHERE LOWER(REPLACE(CONCAT(TRIM(tbl_clientes.nombre_clte), ' ', TRIM(tbl_clientes.apaterno_clte), ' ', TRIM(tbl_clientes.amaterno_clte)), '  ', ' ')) 
        LIKE :nombre
    GROUP BY tbl_clientes.id_cliente";
            }
            ...
            // Procesar la entrada para búsqueda parcial
            $nombreParam = "%" . str_replace(' ', '%', $nombre) . "%";
    ...
    static public function mdlDatosClientes($id)
    {
        try {
            ...
            CONCAT_WS(' ', nombre_clte, apaterno_clte, amaterno_clte) AS nombre_cliente, tbl_clientes.fk_estado,  tbl_clientes.fk_municipio, tbl_clientes.fk_localidad,  nombre_clte, apaterno_clte, amaterno_clte, curp, rfc, fecha_nacimiento, religion, Fk_estado_nac, tipo_identificacion, clave_elector, nom_localidad,
            sexo, tel_celular, tel_casa, tel_oficina, tel_otro,  email, email2 , calle, colonia, ciudad, num_int, num_ext, condicion_migrat, cod_postal, nacimi.nom_pais as pais_nac, nac.nom_estado as estado_nac, nacionali.nom_pais as nacionalidad,
            CONCAT_WS(' ', nombre_cony, apaterno_cony, amaterno_cony) AS nombre_conyugue, amaterno_cony, apaterno_cony, nombre_cony, num_hijos, num_dep_eco, estado_civil,
    ...
    {
        try {
            $stmt = Conexion::conectar()->prepare("SELECT *, CONCAT_WS(' ',nombre_cotitular,apaterno_cotitular,amaterno_cotitular) AS nombre_cotitular, nombre_cotitular AS nom_cotitular,  
             nom_localidad as nom_localidad_cotitular, nacimi.nom_pais as pais_nac_cotitular, nac.nom_estado as estado_nac_cotitular, nacionali.nom_pais as nacionalidad_cotitular,
             desc_municipio as desc_municipio_cotitular, c.nom_estado as nom_estado_cotitular 
    ...
    static public function mdlConsultaDocsPersonal($id)
    {
        try {
            $stmt = Conexion::conectar()->prepare("SELECT *
            FROM tbl_docs_personales
...
GROUP BY tcb.num_contrato, tcb.status_liquidacion, tcb.t_moneda, tcb.fk_empleado;
    ...
    static public function mdlEditCliente(
        $id_cliente,
        $Fk_promotor,
        $tipo_cliente,
        $razon_social,
        ...
        try {
            $conexion = Conexion::conectar();
            $stmt = $conexion->prepare("UPDATE tbl_clientes SET 
                Fk_promotor = :Fk_promotor, tipo_cliente = :Tipo_cliente, razon_social = :Razon_social,
                nombre_clte = :Nombre_clte, apaterno_clte = :Apaterno_clte, amaterno_clte = :Amaterno_clte,
                sexo = :Sexo, tipo_identificacion = :Tipo_identificacion, clave_elector = :Clave_elector,
                curp = :Curp, rfc = :Rfc, fecha_nacimiento = :Fecha_nacimiento, religion = :Religion,
                Fk_pais_nac = :nacionalidadPais, Fk_estado_nac = :Fk_estado_nac, 
                estado_nac_extran = :Estado_nac_extran, nacionalidad = :Nacionalidad, 
                condicion_migrat = :Condicion_migrat, estado_civil = :Estado_civil, 
                nombre_cony = :Nombre_cony, apaterno_cony = :Apaterno_cony, 
                amaterno_cony = :Amaterno_cony, num_hijos = :Num_hijos, num_dep_eco = :Num_dep_eco, 
                tel_celular = :Tel_celular, tel_casa = :Tel_casa, tel_oficina = :Tel_oficina, 
                tel_otro = :Tel_otro, email = :Email, email2 = :Email2, fk_pais = :Fk_pais, 
                fk_estado = :Fk_estado, fk_municipio = :Fk_municipio, fk_localidad = :Fk_localidad, 
                calle = :Calle, num_int = :Num_int, num_ext = :Num_ext, colonia = :Colonia, 
                ciudad = :Ciudad, cod_postal = :Cod_postal
    ...
    static public function mdlUpdateClienteActEco(
        $Id_cliente,
        $Profesion,
        $Otra_profesion,
        $Ocupacion_clte,
        ...
        try {
            ...
            // Preparar la consulta
            $stmt = $conexion->prepare("UPDATE tbl_actividad_economica SET 
                    profesion = :Profesion,
                    otra_profesion = :Otra_profesion,
                    ocupacion_clte = :Ocupacion_clte,
                    puesto_clte = :Puesto_clte,
                    actividad = :Actividad,
                    otra_actividad = :Otra_actividad,
                    nom_empres = :Nom_empresa,
                    tipo_empresa = :Tipo_empresa,
                    otro_tipo_emp = :Otro_tipo_emp,
                    fk_pais = :Fk_pais,
                    fk_estado = :Fk_estado,
                    ...
                    ingre_mensual = :Ingre_mensual,
                    ingresos_provienen = :Ingresos_provienen
            ...
            if ($stmt->execute()) {
                return [
                    'status' => 'success',
                    'message' => 'Actividad económica actualizada correctamente'
                ];
            } else {
                return [
                    'status' => 'error',
                    'message' => 'Error al actualizar actividad económica'
                ];
            }
        } catch (PDOException $e) {
            return [
    ...
    static public function mdlInsertTransaccionalidad(
        $Id_cliente,
        $recursos,
        $num_porciento_propio,
        $num_porciento_tercero_glob,
        $uso_cuenta,
        $rec_esp,
        $transac_mens,
        $mont_mens,
    ...
    static public function mdlUpdateTransaccionalidad(
        $Id_cliente,
        $recursos,
        $num_porciento_propio,
        $num_porciento_tercero_glob,
        ...
        try {
            ...
            $stmt = $conexion->prepare("UPDATE tbl_transaccionalidad 
                SET 
                    recursos = :recursos,
                    propios = :num_porciento_propio,
                    terceros = :num_porciento_tercero_glob,
                    uso_cuenta = :uso_cuenta,
                    uso_cuenta_otro = :rec_esp,
                    transac_men = :transac_mens,
                    monto_men = :mont_mens,
                    saldo_men = :saldo_mens
                WHERE Fk_cliente = :Id_cliente");
            ...
            if ($stmt->execute()) {
                return [
                    'status' => 'success',
                    'message' => 'Transaccionalidad actualizada correctamente'
                ];
            } else {
                $errorInfo = $stmt->errorInfo();
                return [
                    'status' => 'error',
                    'message' => 'Error al actualizar Transaccionalidad: ' . $errorInfo[2]
                ];
            }
        } catch (PDOException $e) {
            return [
    ...
    static public function mdlBorrarBeneficiarios(
        $fk_cliente
       
    ) {
        try {
            $conexion = Conexion::conectar();
            $stmt = $conexion->prepare("DELETE FROM tbl_beneficiarios 
                WHERE id_beneficiario IN (
                SELECT fk_beneficiario 
                FROM rel_ben_venta 
                WHERE fk_solSeg = :id_cliente
                );

                  DELETE FROM rel_ben_venta 
                WHERE fk_solSeg = :id_cliente;
                ");
           ...
           ;
            ...
            if ($stmt->execute()) {
                ...
                return [
                    'status' => 'success',
                    'message' => 'Documentos  Insertados'

                ];
            } else {
                return [
                    'status' => 'error',
                    'message' => 'Error al insertar Documentos'
                ];
            }
        } catch (PDOException $e) {
            return [
    ...
    static public function mdlInsertDocsPersonales(
        $fk_cliente,
        $ine_anverso,
        $ine_reverso,
        $domicilio_img,
        ...
        try {
            $conexion = Conexion::conectar();
            $stmt = $conexion->prepare("INSERT INTO `tbl_docs_personales` ( `fk_cliente`, `ine_anverso`, `ine_reverso`, `domicilio_img`, 
            `curp_img`, `edo_cuenta_img`, `situacion_fiscal_img`, `cuestionario_inver_img`, `ine_anverso_cotitutar`, `ine_reverso_cotitular`,
             `curp_cotitular_img`, `situacion_fiscal_cotitular_img`, `fech_up`) 
             VALUES ( :fk_cliente, :ine_anverso, :ine_reverso, :domicilio_img, 
            :curp_img, :edo_cuenta_img, :situacion_fiscal_img, :cuestionario_inver_img, :ine_anverso_cotitutar, :ine_reverso_cotitular,
             :curp_cotitular_img, :situacion_fiscal_cotitular_img, :fech_up;");

            // Vincular parámetros
            $stmt->bindParam(':fk_cliente', $fk_cliente, PDO::PARAM_INT);
            $stmt->bindParam(':ine_anverso', $ine_anverso, PDO::PARAM_STR);
            $stmt->bindParam(':ine_reverso', $ine_reverso, PDO::PARAM_STR);
            $stmt->bindParam(':domicilio_img', $domicilio_img, PDO::PARAM_STR);
            $stmt->bindParam(':curp_img', $curp_img, PDO::PARAM_STR);
            $stmt->bindParam(':edo_cuenta_img', $edo_cuenta_img, PDO::PARAM_STR);
            $stmt->bindParam(':situacion_fiscal_img', $situacion_fiscal_img, PDO::PARAM_STR);
            $stmt->bindParam(':cuestionario_inver_img', $cuestionario_inver_img, PDO::PARAM_STR);
            $stmt->bindParam(':fech_up', $fech_up, PDO::PARAM_STR);
            $stmt->bindParam(':ine_anverso_cotitutar', $ine_anverso_cotitutar, PDO::PARAM_STR);
            $stmt->bindParam(':ine_reverso_cotitular', $ine_reverso_cotitular, PDO::PARAM_STR);
            $stmt->bindParam(':curp_cotitular_img', $curp_cotitular_img, PDO::PARAM_STR);
            $stmt->bindParam(':situacion_fiscal_cotitular_img', $situacion_fiscal_cotitular_img, PDO::PARAM_STR);
    ...
    static public function mdlUpdateDocsPersonales(
        $fk_cliente,
        $ine_anverso,
        $ine_reverso,
        $domicilio_img,
        ...
        try {
            $conexion = Conexion::conectar();
            $stmt = $conexion->prepare("UPDATE `tbl_docs_personales` 
                SET `ine_anverso` = :ine_anverso,
                    `ine_reverso` = :ine_reverso,
                    `domicilio_img` = :domicilio_img,
                    `curp_img` = :curp_img,
                    `edo_cuenta_img` = :edo_cuenta_img,
                    `situacion_fiscal_img` = :situacion_fiscal_img,
                    `cuestionario_inver_img` = :cuestionario_inver_img,
                    `ine_anverso_cotitutar` = :ine_anverso_cotitutar,
                    `ine_reverso_cotitular` = :ine_reverso_cotitular,
                    `curp_cotitular_img` = :curp_cotitular_img,
                    `situacion_fiscal_cotitular_img` = :situacion_fiscal_cotitular_img,
                    `fech_up` = :fech_up
    ...
    static public function mdlInsertarCotitular(
        ...
        $claveElectorCotitular,
        $curpCotitular,
        $rfcCotitular,
        $fNacCotitular,
        $religion_cotitular,
        $Fk_pais_nac_cotitular,
        $Fk_estado_nac_cotitular,
        $nacionalidad_cotitular,
        $condicion_migrat_cotitular,
        ...
        $fk_pais_cotitular,
        $fk_estado_cotitular,
        $fk_municipio_cotitular,
        $fk_localidad_cotitular,
        $calle_cotitular,
        $num_ext_cotitular,
        $num_int_cotitular,
        $colonia_cotitular,
